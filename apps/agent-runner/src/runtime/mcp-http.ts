...
// Shell tools — execSync used intentionally for CLI wrappers
server.registerTool("git.status", { description: "Git status", inputSchema: {} }, async () => { try { return ok({ stdout: execSync("git status --porcelain", { encoding: "utf-8", timeout: 10000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
server.registerTool("git.diff", { description: "Git diff", inputSchema: {} }, async () => { try { return ok({ stdout: execSync("git diff --stat && git diff", { encoding: "utf-8", timeout: 15000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
server.registerTool("git.commit", { description: "Git commit", inputSchema: { message: z.string() } }, async ({ message }) => { try { return ok({ stdout: execSync(`git commit -m "${message.replace(/"/g, '\\"')}"`, { encoding: "utf-8", timeout: 15000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
server.registerTool("npm.install", { description: "NPM install", inputSchema: { packages: z.array(z.string()).optional() } }, async ({ packages }) => { const cmd = packages?.length ? `pnpm add ${packages.join(" ")}` : "pnpm install"; try { return ok({ stdout: execSync(cmd, { encoding: "utf-8", timeout: 120000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
server.registerTool("npm.run", { description: "NPM run script", inputSchema: { script: z.string() } }, async ({ script }) => { try { return ok({ stdout: execSync(`pnpm run ${script}`, { encoding: "utf-8", timeout: 120000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
server.registerTool("docker.build", { description: "Docker build", inputSchema: { tag: z.string().optional() } }, async ({ tag }) => { const cmd = tag ? `docker build -t ${tag} .` : "docker build ."; try { return ok({ stdout: execSync(cmd, { encoding: "utf-8", timeout: 300000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
server.registerTool("docker.run", { description: "Docker run", inputSchema: { image: z.string() } }, async ({ image }) => { try { return ok({ stdout: execSync(`docker run --rm ${image}`, { encoding: "utf-8", timeout: 300000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
server.registerTool("docker.ps", { description: "Docker ps", inputSchema: {} }, async () => { try { return ok({ stdout: execSync("docker ps --format json", { encoding: "utf-8", timeout: 10000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
server.registerTool("cicd.pipeline_status", { description: "CI/CD pipeline status", inputSchema: {} }, async () => { try { return ok({ pipelines: JSON.parse(execSync("gh run list --limit 5 --json name,status,conclusion,createdAt", { encoding: "utf-8", timeout: 15000 })), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ pipelines: [], exitCode: err.status ?? 1 }); } });
server.registerTool("cicd.pipeline_trigger", { description: "Trigger CI/CD pipeline", inputSchema: { pipeline: z.string() } }, async ({ pipeline }) => { try { return ok({ triggered: true, stdout: execSync(`gh workflow run ${pipeline}`, { encoding: "utf-8", timeout: 30000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ triggered: false, exitCode: err.status ?? 1 }); } });
server.registerTool("cicd.runs_view", { description: "View CI/CD runs", inputSchema: {} }, async () => { try { return ok({ runs: JSON.parse(execSync("gh run list --limit 20 --json name,status,conclusion,headBranch,createdAt,databaseId", { encoding: "utf-8", timeout: 15000 })), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ runs: [], exitCode: err.status ?? 1 }); } });
server.registerTool("github.pr_create", { description: "Create GitHub PR", inputSchema: { title: z.string(), body: z.string().optional() } }, async ({ title, body }) => { try { return ok({ created: true, stdout: execSync(`gh pr create --title "${title.replace(/"/g, '\\"')}" --body "${(body ?? "").replace(/"/g, '\\"')}"`, { encoding: "utf-8", timeout: 30000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ created: false, stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
server.registerTool("github.pr_list", { description: "List GitHub PRs", inputSchema: {} }, async () => { try { return ok({ prs: JSON.parse(execSync("gh pr list --json number,title,state,author,createdAt", { encoding: "utf-8", timeout: 15000 })), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ prs: [], exitCode: err.status ?? 1 }); } });
server.registerTool("github.issue_list", { description: "List GitHub issues", inputSchema: {} }, async () => { try { return ok({ issues: JSON.parse(execSync("gh issue list --json number,title,state,labels,createdAt", { encoding: "utf-8", timeout: 15000 })), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ issues: [], exitCode: err.status ?? 1 }); } });
server.registerTool("tests.run", { description: "Run tests", inputSchema: { pattern: z.string().optional() } }, async ({ pattern }) => { const cmd = pattern ? `pnpm test -- ${pattern}` : "pnpm test"; try { return ok({ stdout: execSync(cmd, { encoding: "utf-8", timeout: 120000 }), exitCode: 0 }); } catch (e: unknown) { const err = e as { stdout?: string; stderr?: string; status?: number }; return ok({ stdout: err.stdout ?? "", stderr: err.stderr ?? "", exitCode: err.status ?? 1 }); } });
