...
export default function ChatPage() {
  ...
  return (
    <AppShell>
      <div className="max-w-3xl mx-auto h-[calc(100vh-5rem)] md:h-[calc(100vh-8rem)] flex flex-col">
        <div className="rounded-none md:rounded-xl border-0 md:border border-[var(--border)] bg-[var(--bg-card)] flex-1 flex flex-col overflow-hidden">
          <div className="px-3 md:px-4 py-2 md:py-3 border-b border-[var(--border)]">
            <h2 className="text-sm font-medium text-[var(--text-primary)]">Chat with AI Agent</h2>
            <p className="text-xs text-[var(--text-secondary)] hidden md:block">Uses LLM providers with memory context</p>
          </div>
          <ChatInterface
            onSend={handleSend}
            messages={messages}
