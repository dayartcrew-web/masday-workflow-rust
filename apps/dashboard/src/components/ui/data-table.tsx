        ...
        <thead>
          <tr style={{ background: 'var(--color-surface-elevated)', borderBottom: '1px solid var(--color-border-subtle)' }}>
            {columns.map((col) => (
              <th
                key={col.key}
                className="text-left py-3 px-4 text-[var(--color-text-secondary)] font-medium whitespace-nowrap uppercase tracking-wider"
        ...
        <tbody>
          {paged.length === 0 ? (
            ...
            paged.map((item, idx) => (
              ...
              >
                {columns.map((col) => (
                  <td key={String(col.key)} className="py-2 px-4 text-[var(--color-text)]">
