---
name: masday-worktree
description: Manage per-task git worktrees for isolated implementation
version: 1.0.0
---
...
- Git repo exists
- Active workflow with a task loaded (`mcp__masday__workflow_getCurrentTask`)
...
Step 1: Get task info
  ...
  mcp__masday__workflow_getCurrentTask({ workflow_id: workflowId })
...
Step 2: Generate branch name
  ...
  branch = "task/{slug}"
  worktreeDir = ".masday/worktrees/{slug}"
...
Step 4: Record worktree info
  Write to .masday/worktrees/{slug}.json:
  {
    "taskId": "{task.id}",
...
Step 5: Report
  ...
  Print: "Implement in that directory. Use /masday-worktree done when finished."
```
...
Step 1: Find active worktree
  Read .masday/worktrees/{slug}.json (find the one matching current task)
...
```
  ...
  Update .masday/worktrees/{slug}.json:
...
```
  ...
  Report: "Run /masday-worktree done again when ready to merge."
```
...
```
Step 1: Glob: .masday/worktrees/*.json
Step 2: Print table:
