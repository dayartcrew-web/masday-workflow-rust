...
pub fn update_global_settings(settings_path: &Path, updates: &SettingsUpdates) -> Result<()> {
    ...
    let obj = json.as_object_mut().expect("Root should be an object");
    ...
    if let Some(ref mcp_config) = updates.mcp_server {
        ...
        server_obj.insert(
            "command".to_string(),
            JsonValue::String(mcp_config.command.clone()),
        );
...
pub fn update_json_config(path: &Path, key: &str, value: JsonValue) -> Result<()> {
    ...
    for (i, &k) in keys.iter().enumerate() {
        if i == keys.len() - 1 {
            ...
            let key_exists = current.as_object().and_then(|o| o.get(k)).is_some();
...
mod tests {
    ...
    fn test_update_global_settings_new_file() {
        ...
        let updates = SettingsUpdates {
            statusline_cmd: Some("/path/to/hook.js".to_string()),
