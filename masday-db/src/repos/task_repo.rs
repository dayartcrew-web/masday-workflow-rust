...
impl TaskRepo {
    ...
    pub async fn create(&self, task: &NewTask) -> Result<Task> {
        ...
        let row = client.query_one(
            query,
            &[
                &id, &task.workflow_id, &task.plan_id, &task.title, &task.status,
                &task.priority, &task.owner_agent, &task.acceptance_criteria,
                &task.required_context, &task.verification_steps, &task.context_fingerprint,
                &task.progress_percent, &task.requires_tdd, &task.test_evidence,
                &now, &now,
            ],
        ).await.map_err(|e| AppError::Database(format!("Failed to create task: {}", e)))?;
    ...
    pub async fn complete(&self, id: &str, result: serde_json::Value) -> Result<Task> {
        ...
        let query = "
            ...
            RETURNING *
        ";
        let row = client.query_one(query, &[&result, &now, &id]).await
    ...
    pub async fn save_progress(&self, log: &NewTaskProgressLog) -> Result<TaskProgressLog> {
        ...
        let row = client.query_one(
            query,
            &[
                &id, &log.workflow_id, &log.task_id, &log.agent_name,
                &log.status_before, &log.status_after, &log.progress_note,
                &log.evidence, &now,
            ],
        ).await.map_err(|e| AppError::Database(format!("Failed to save progress log: {}", e)))?;
