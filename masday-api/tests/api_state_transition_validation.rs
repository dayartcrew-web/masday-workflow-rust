...
async fn test_valid_state_transitions_via_service() {
    ...
    let workflow = create_test_workflow("valid-transitions-test")
        .await
    ...
    .await
    .expect("INIT → ANALYZE should succeed");
    ...
    .await
    .expect("ANALYZE → PLAN should succeed");
    ...
    .await
    .expect("PLAN → PAUSED should succeed");
    ...
    .await
    .expect("PAUSED → EXECUTE should succeed");
...
async fn test_invalid_state_transition_done_to_execute() {
    ...
    let workflow = create_test_workflow("invalid-done-to-execute")
        .await
        .expect("Failed to create workflow");
    let workflow_id = workflow.id.clone();
    ...
    match result {
        Err(e) => {
            ...
            assert!(
                error_msg.contains("Invalid state transition"),
...
async fn test_invalid_status_string_rejected() {
    ...
    let workflow = create_test_workflow("invalid-status-string")
        .await
        .expect("Failed to create workflow");
    let workflow_id = workflow.id.clone();

    // Test that invalid status strings are rejected at conversion time
    let invalid_statuses = vec!["INVALID_STATE", "NOT_A_REAL_STATE", "RandomText", "", "123"];
    ...
    for invalid_status in invalid_statuses {
        ...
        assert!(
            result.is_err(),
            "status_to_state should reject invalid status '{}'",
            invalid_status
        );
        ...
        match result {
            Err(e) => {
                let error_msg = e.to_string();
                assert!(
                    error_msg.contains("Invalid workflow state")
    ...
    assert!(
        result.is_ok(),
        "status_to_state should accept lowercase 'init'"
    );
...
async fn test_status_to_state_all_valid_states() {
    ...
    for (status_str, expected_state) in &valid_states {
        ...
        assert!(
            result.is_ok(),
            "status_to_state should accept '{}'",
            status_str
        );

        let state = result.unwrap();
        assert!(
            std::mem::discriminant(&state) == std::mem::discriminant(expected_state),
            "status_to_state should convert '{}' to {:?}",
            status_str,
            expected_state
        );
    }
    ...
    for (status_str, expected_state) in &valid_states {
        ...
        assert!(
            result.is_ok(),
            "status_to_state should accept lowercase '{}'",
            lowercase
        );
