...
export function flushEarlyBuffer(): void {
  if (!drizzleDb || !schemaTables || earlyBuffer.length === 0) return;
  const toFlush = [...earlyBuffer];
  earlyBuffer.length = 0;
  logger.info({ count: toFlush.length }, 'Flushing early buffer to PostgreSQL');
  for (const wf of toFlush) {
    replicateWorkflowStatic(wf).catch((err: unknown) => {
      logger.warn({ err: String(err), workflowId: wf.id }, 'Early buffer flush failed');
    });
  }
}

async function replicateWorkflowStatic(workflow: Workflow): Promise<void> {
  ...
  for (const task of workflow.tasks) {
    await replicateTaskStatic(workflow.id, task);
  }
}

async function replicateTaskStatic(workflowId: string, task: Task): Promise<void> {
  if (!drizzleDb || !schemaTables) return;
 ...
 * Buffers writes when DB isn't ready yet, flushes on flushEarlyBuffer().
 */
const MAX_PENDING = 50;
const WARN_PENDING = 25;
...
export class DualWriteWorkflowStore implements IWorkflowStore {
  ...
  save(workflow: Workflow): void {
    ...
    if (this.pendingCount >= MAX_PENDING) {
      logger.warn({ pendingCount: this.pendingCount, max: MAX_PENDING, workflowId: workflow.id }, 'Replication queue full — dropping write');
      return;
    }
    this.pendingCount++;
    if (this.pendingCount >= WARN_PENDING) {
      logger.warn({ pendingCount: this.pendingCount, workflowId: workflow.id }, 'Replication queue depth high');
    }
    this.pendingReplication = this.pendingReplication.then(() => replicateWorkflowStatic(workflow)).catch((err: unknown) => {
