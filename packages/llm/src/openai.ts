/**
 * OpenAI-compatible LLM provider using raw fetch.
 *
 * Works with any OpenAI-compatible API (OpenAI, Azure OpenAI, etc.).
 * - Standard chat completions format.
 * - Response: data.choices[0].message.content
 * - Token usage: data.usage.total_tokens
 */
...
export class OpenAIProvider implements ILLMProvider {
  ...
  async chat(messages: ChatMessage[], options?: LLMOptions): Promise<LLMResponse> {
    ...
    logger.debug({ model, messageCount: messages.length }, 'OpenAI: sending request');
